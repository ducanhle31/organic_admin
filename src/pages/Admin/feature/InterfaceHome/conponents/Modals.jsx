...
function Modals({ tabsData, tabs, isChange, setIsChange }) {
  ...
  const getUrlFirebase = async (file, storage) => {
    const storageRef = ref(storage, file?.name);
    ...
    try {
      ...
      if (file === files.file1) {
        ...
        setUrlImage({ ...urlImage, url1: url });
      } else {
        setFiles({ ...files, file2: null });
        setUrlImage({ ...urlImage, url2: url });
      }
    } catch (error) {
  ...
  const handleConfirm = async () => {
    ...
    if (files.file1 && files.file2) {
      await getUrlFirebase(files.file1, storage);
      await getUrlFirebase(files.file2, storage);
    }
  };

  const handleUploadImage = async () => {
    setLoadingUpload(true);
    if (!urlImage.url1 || !urlImage.url2) {
      setLoadingUpload(false);
      return;
    }
    ...
    try {
      await homeApi.addSlider({
        ...
        name,
        article_date: getTimeCurrent(),
        image_slider_desktop: urlImage.url1,
        image_slider_mobile: urlImage.url2,
      });
          ...
          <Typography component={"span"} id="modal-modal-description" sx={{ mt: 2 }}>
            <Typography component={"span"} className="row">
              {Array(2)
                ...
                .map((value, i) => (
                  <Typography component={"span"} className="col-5" key={i}>
                    ...
                    <Image
                      className={"modals-image"}
                      srcImage={i === 0 ? urlImage.url1 || "assets/image/admin/NoImage.png" : urlImage.url2 || "assets/image/admin/NoImage.png"}
                    />
                    <Typography component={"span"} className="btn-add-image">
                      <Input
                        type="file"
              ...
              <Typography component={"span"} className="modal-save">
                ...
                {(loadingUpload && <BtnLoading btnName={"Uploads"} />) ||
                  (urlImage.url1 && urlImage.url2 && !checkUpload ? (
                    <Button variant="outlined" onClick={handleUploadImage}>
                      Uploads
                    </Button>
                  ) : (
                    <Button variant="outlined" onClick={handleUploadImage} disabled>
                      Uploads
                    </Button>
                  ))}
              </Typography>
